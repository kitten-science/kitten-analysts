...
export class KittenAnalysts {
	...
	connect(withAnalyticsBackend: boolean) {
		...
		this.ws.onerror = (error) => {
			console.warn(
				...cl(
					"Error on WS connection! Closing and reconnecting...",
					error.type,
				),
			);
	...
	processMessage(
		...
		switch (message.type) {
			case "connected":
				break;
			case "getBuildings": {
				const bonfire: PayloadBuildings = game.bld.meta[0].meta.flatMap(
					(building) => {
						if (building.stages) {
				...
				const religion: PayloadBuildings = game.religion.meta.flatMap(
					(meta, index) =>
				...
				const space: PayloadBuildings = game.space.meta.flatMap(
					(meta, index) =>
				...
				const data: PayloadCalendar = [
					{
						...
						futureSeasonTemporalParadox:
							game.calendar.futureSeasonTemporalParadox,
			...
			case "getResourcePool": {
				...
				const resources: PayloadResources = this.game.resPool.resources.map(
					(resource) => {
						let rate =
							(isTimeParadox
								? 0
								: this.game.getResourcePerTick(resource.name, true)) *
							this.game.getTicksPerSecondUI();
						if (resource.calculatePerDay === true) {
							rate =
								this.game.getResourcePerDay(resource.name) *
								(resource.name === "necrocorn"
									? 1 + this.game.timeAccelerationRatio()
									: 1);
			...
			case "getStatistics": {
				const data: PayloadStatistics = game.stats.statGroups.flatMap(
					(group, index) =>
			...
			case "getTechnologies": {
				const data: PayloadTechnologies = game.science.techs
					.filter(
						(tech) =>
							!TechnologiesIgnored.includes(tech.name as TechnologyIgnored),
					)
					.map((tech) => ({
						label: tech.label,
						name: tech.name,
	...
	postMessage<TMessage extends KittenAnalystsMessageId>(
		...
		try {
			...
			console.warn(
				...cl("Error while sending message. Closing socket.", error),
			);
