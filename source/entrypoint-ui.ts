...
const PORT_HTTP_KG = process.env.PORT_HTTP_KG
	? Number(process.env.PORT_HTTP_KG)
	: 8080;

const main = async () => {
	const dom = await JSDOM.fromURL(
		`http://${HOSTNAME_KG}:${PORT_HTTP_KG}/headless.html`,
		{
	...
	try {
		...
		process.stderr.write(
			`File contents: '${value.substring(0, 30).replaceAll("\n", "")}'\n`,
		);
		...
		process.stderr.write(
			`Cloud save save data is typeof: '${typeof cloudSave.saveData}'\n`,
		);
		...
		process.stderr.write(
			`Setting '${lsEntry}' to save game from '${ephemeralPath}'...\n`,
		);
		dom.window.localStorage.setItem(lsEntry, saveDataString);
	} catch (_error) {
		process.stderr.write(
			`Unable to read save state from '${ephemeralPath}'.\n`,
		);
	}
